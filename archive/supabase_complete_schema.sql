-- ============================================================================
-- Ionexa AI — COMPLETE consolidated Supabase schema
--
-- This file is the full, current database schema for the entire project,
-- assembled by concatenating the two live schema files in the order they
-- must be applied:
--   1) supabase_schema.sql          (all 13 core modules + every V2/V3
--                                     table: AI Company/Mission Control,
--                                     Website Builder incl. versioning,
--                                     reference images, security_check_log,
--                                     Trading Workflow, Gamification,
--                                     Automations, Scheduled Agent Runs,
--                                     Team/beta/rate-limit/circuit-breaker
--                                     infrastructure, etc.)
--   2) supabase_credits_schema.sql  (billing: user_credits,
--                                     credit_transactions, and the
--                                     deduct_credits_atomic RPC)
--
-- Both source files are individually idempotent (safe to re-run against a
-- project that already has some or all of this applied — every statement
-- uses IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS as appropriate).
-- Running this single file end-to-end on a brand-new Supabase project
-- reproduces the exact schema the app expects, in one pass.
--
-- *** CRITICAL — supabase_schema.sql (Part 1 below) starts with a series
-- of `drop table if exists ... cascade` statements covering most tables
-- in this app, including ones that hold real production data once the
-- app is live (ai_missions, user_websites, chat_conversations, and more
-- — see the full warning at the top of Part 1). DO NOT re-run this
-- consolidated file against a live production database — it is a
-- "fresh project, run once" script. Every change made after initial
-- setup should instead be a small, additive, idempotent `alter table
-- ... add column if not exists` statement, the same pattern used
-- throughout both source files for every table added after the
-- original 13 modules. ***
--
-- The standalone `*_migration.sql` files at the repo root (
-- website_status_migration.sql, website_reliability_migration.sql,
-- website_reference_images_migration.sql, user_automations_migration.sql,
-- scheduled_agent_runs_migration.sql, daily_ai_spend_tracking_migration.sql
-- ) are historical incremental patches from earlier passes — every table,
-- column, and constraint they add is already included directly below (in
-- supabase_schema.sql section), so they are redundant against a project
-- that runs this consolidated file. They're kept in the repo only for
-- reference / for anyone who already ran them individually against a live
-- database before this consolidation existed.
--
-- supabase_full_project_backup.sql and supabase_v2_consolidated.sql are
-- earlier point-in-time snapshots from prior passes and are now stale —
-- this file supersedes both.
--
-- This file is generated by concatenation; supabase_schema.sql and
-- supabase_credits_schema.sql remain the actual sources of truth that get
-- edited going forward. Regenerate this file after any change to either.
-- ============================================================================

-- ============================================================================
-- PART 1 / 2 — supabase_schema.sql
-- ============================================================================
...
-- ============================================================================
-- PART 2 / 2 — supabase_credits_schema.sql
-- ============================================================================
