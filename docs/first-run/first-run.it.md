...
Everything a new person reads from the signup form to the first thing the product tells them about their own data: **597 strings**. The whole product is 3119, which is why this file exists.
...
Non siamo riusciti a creare l'account. Controlla i dati e riprova — non ti è stato addebitato nulla.
...
Non siamo riusciti ad accedere. Controlla email e password, oppure reimposta la password se non sei sicuro.

**`auth.login.oauthFailed`**

> EN — That sign-in didn't complete. Try again, or use your email and password below.

L'accesso non è stato completato. Riprova oppure usa email e password qui sotto.
...
Porta dati reali e l'IA ti dirà qualcosa sulla tua attività entro due minuti.
...
Solo schemi reali da ciò che hai appena importato. Se non basta per esserne certi, te lo diremo.
...
CSV o separato da tabulazioni, fino a {max}. Lo leggiamo e ti mostriamo cosa abbiamo trovato prima di salvare.
...
Le tue date potrebbero essere giorno/mese o mese/giorno — cadono tutte entro il 12, quindi non possiamo saperlo. Quale delle due?

**`dashboard.onboarding.firstFree`**

> EN — Your first import and analysis are free — they will not use any credits.

Il primo import e la relativa analisi sono gratuiti — non consumeranno crediti.
...
Non c'è ancora abbastanza per parlare di uno schema. Qualche decina di righe con le date è di solito il punto in cui iniziano a emergere cose — e preferiamo tacere piuttosto che inventare.
...
Un business plan, appunti di riunione, un elenco clienti. Estraiamo ciò che è registrabile e lasciamo il resto.

**`dashboard.onboarding.sourceCsvHint`**

> EN — A CSV export from your broker, bank or CRM. We work out what each column is.

Un export CSV dal tuo broker, banca o CRM. Capiamo cos'è ogni colonna.

**`dashboard.onboarding.sourceIntro`**

> EN — Pick whichever is easiest. Nothing here is required, and you can add more later.

Scegli ciò che ti è più comodo. Nulla è obbligatorio e puoi aggiungere altro dopo.

**`dashboard.onboarding.sourcePasteHint`**

> EN — A business plan, notes, a list — we pull the structured bits out.

Un business plan, appunti, un elenco — ne estraiamo le parti strutturate.
...
Non registri nulla da un po' — aggiungi una nuova voce per riprendere.

**`dashboard.overview.nextAction.revisitLink`**

> EN — You linked "{source}" to "{target}" a few days ago — worth revisiting?

Hai collegato "{source}" a "{target}" qualche giorno fa — vale la pena rivederlo?

**`dashboard.overview.nextAction.startNew`**

> EN — No new activity in the last 3 days — ready to start something new?

Nessuna nuova attività negli ultimi 3 giorni — pronto a iniziare qualcosa di nuovo?
...
Il tuo punteggio di attività appare dopo {count} voci registrate: abbastanza perché nessuna singola lo decida.

**`dashboard.overview.statRow.mostActiveExplain`**

> EN — The module you have written in most. Where your attention has gone.

Il modulo in cui scrivi di più. Dove va la tua attenzione.

**`dashboard.overview.statRow.thisWeekExplain`**

> EN — Logged in the last seven days — how active this week has been.

Registrato negli ultimi sette giorni: quanto è stata attiva la settimana.

**`common.betaExpiry`**

> EN — Your beta access expires in {days, plural, one {# day} other {# days}}. <link>Upgrade to keep full access</link>.

Il tuo accesso beta scade tra {days, plural, one {# giorno} other {# giorni}}. <link>Passa a un piano superiore per mantenere l'accesso completo</link>.

**`common.listCapped`**

> EN — Showing the most recent {count, number}. Older entries are still saved — use Search my records to find them.

Sono mostrate le {count, number} più recenti. Le voci più vecchie restano salvate: usa Cerca nei miei dati per trovarle.

**`dashboard.create.looksLikeQuestion`**

> EN — That looks like a question. Should I answer it, or record it?

Sembra una domanda. La rispondo o la registro?
...
Ionexa lo usa per scegliere quale passo del piano proporti — lavoro leggero quando sei giù, impegnativo quando non lo sei.
...
Seleziona il tuo paese (opzionale)
...
Scegli una password che soddisfi tutti i requisiti sopra indicati.
...
Scegli Professional o Ultimate come piano base

**`pricing.businessFeatureFreeOnUltimate`**

> EN — Team seats included free on Ultimate

Posti team inclusi gratuitamente con Ultimate
...
Gestisci i posti in qualsiasi momento dalle impostazioni Team

**`pricing.businessSubtitle`**

> EN — For teams building together

Per team che costruiscono insieme
...
{count, plural, one {# credito} other {# crediti}}/mese

**`pricing.features.customAiPersonaNameInIonexaChat`**

> EN — Custom AI persona name in Ionexa Chat

Nome personalizzato dell'assistente in Ionexa Chat
...
Tutto ciò che include Professional
...
Accesso a Website & Automation Builder
...
Accedi per salvare ciò che hai condiviso.
...
{ready} righe su {total} sono pronte
...
Una startup che sto costruendo
...
Cosa vuoi tenere sotto controllo soprattutto?
...
{count, plural, one {Importa # riga} other {Importa # righe}}
...
Cosa verrà effettivamente salvato
...
Vai dritto alla dashboard e parti da zero.
...
I fogli di calcolo devono essere al massimo {max}.

**`dashboard.onboarding.truncated`**

> EN — only the first rows were read

sono state lette solo le prime righe

**`promise.oneSentence`**

> EN — The AI that already knows your work. Ask it anything.

L'IA che conosce già il tuo lavoro. Chiedile qualsiasi cosa.
...
Crediti rimanenti — acquistane altri nelle Impostazioni
...
{message} — premi Invio per ignorare

**`common.jumpToPage`**

> EN — Jump to a module or page...

Vai a un modulo o pagina...
...
Niente in questa pagina si sta aggiornando.

**`common.offline.showingCachedAge`**

> EN — Nothing here is updating — this was loaded {minutes} min ago.

Qui non si aggiorna nulla: caricato {minutes} min fa.
...
Accesso proprietario — crediti illimitati
...
Messaggio gratuito · {count} rimasti questo mese

**`credits.unlimited`**

> EN — Unlimited — no credits used

Illimitato — nessun credito usato

**`credits.unlimitedWouldHaveCost`**

> EN — Unlimited — would have cost {count, plural, one {# credit} other {# credits}}

Illimitato — sarebbe costato {count, plural, one {# credito} other {# crediti}}

**`credits.used`**

> EN — {count, plural, one {Used # credit} other {Used # credits}}

{count, plural, one {Usato # credito} other {Usati # crediti}}

**`credits.usedWithRemaining`**

> EN — {count, plural, one {Used # credit} other {Used # credits}} · {remaining} left

{count, plural, one {Usato # credito} other {Usati # crediti}} · {remaining, plural, one {# rimasto} other {# rimasti}}
...
Non è stato possibile caricare le tue idee: {message}
...
Grazie per aver testato Ionexa AI. Il tuo feedback è benvenuto.

**`dashboard.overview.healthScore.buildingMomentum`**

> EN — Building momentum

Stai prendendo slancio
...
Continua: {step} dal tuo piano "{goal}"
...
Quel che resta della tua quota mensile per il lavoro con l'IA.
...
{count, plural, one {su # in totale} other {su # in totale}}
...
Tutto ciò che hai registrato, in ogni modulo, dall'inizio.
...
Descrivi la tua idea nel dettaglio...
...
Crea un sito per il mio negozio
...
Cosa dicono i miei numeri questa settimana?
...
Vedi la cronologia dei crediti
...
da {count, plural, one {# delle tue voci} other {# delle tue voci}}
...
ricarica ora per non interromperti.

**`credits.low.none`**

> EN — No credits left this month

Nessun credito rimasto questo mese

**`credits.low.remaining`**

> EN — {count, plural, one {# credit left} other {# credits left}} this month

{count, plural, one {# credito rimasto} other {# crediti rimasti}} questo mese
...
Non è stato possibile salvare la lingua: non è stato modificato nulla.
...
Mettilo nella schermata Home: schermo intero e notifiche che arrivano davvero.
...
Safari non lo propone mai da solo: bastano tre tocchi.
...
Tocca il pulsante Condividi nella barra di Safari
...
Tocca Aggiungi — Ionexa comparirà con le altre app

**`pwa.iosTitle`**

> EN — Add Ionexa to your Home Screen

Aggiungi Ionexa alla schermata Home

**`pwa.iosWhy`**

> EN — Until you do, iPhone cannot send you notifications, and Safari may clear your saved work after 7 unused days.

Fino ad allora l'iPhone non può inviarti notifiche e Safari potrebbe cancellare i dati salvati dopo 7 giorni di inutilizzo.
...
Questa azione richiede più crediti di quelli rimasti. Acquista un pacchetto o passa a un piano superiore per continuare.
...
Ti restano {available} crediti e questo ne richiede circa {needed}. Acquista un pacchetto o passa a un piano superiore.
...
Un nuovo servizio per le piccole imprese

**`dashboard.ideas.empty.title`**

> EN — Every idea, in one place

Ogni idea, in un unico posto
...
Annotala finché è ancora grezza: qui viene valutata, confrontata con le altre e resta anche se la scarti.
...
com'è il prodotto minimo funzionante?
...
Il tuo dispositivo non è riuscito a raggiungerci.

**`errors.codes.planLimit.next`**

> EN — Delete something you no longer need, or upgrade in Settings › Billing.

Elimina qualcosa che non ti serve più, oppure passa a un piano superiore da Impostazioni › Fatturazione.
...
Aspetta circa un minuto e riprova.
...
Dipende da noi e di solito si risolve in pochi minuti.

**`errors.codes.upstreamUnavailable.what`**

> EN — The AI service isn't responding right now.

Il servizio di IA non risponde in questo momento.

**`errors.credits.charged`**

> EN — This attempt used credits.

Questo tentativo ha consumato crediti.
...
I tuoi crediti sono stati restituiti.

**`errors.credits.unverified`**

> EN — We can't confirm from here whether this was charged.

Da qui non possiamo confermare se sia stato addebitato.
...
{credits, plural, one {# credito} other {# crediti}} al minuto di parlato
...
Non è stato inviato nulla. Correggi il testo e invialo tu.

**`voice.draft.title`**

> EN — What was heard

Ciò che è stato sentito
...
Non è stato possibile leggere quella richiesta.

**`voice.errors.capacity`**

> EN — The service is busy right now. Try again shortly.

Il servizio è molto occupato in questo momento. Riprova tra poco.

**`voice.errors.denied`**

> EN — The microphone was not allowed. You can still type.

Il microfono non è stato autorizzato. Puoi sempre scrivere.

**`voice.errors.empty`**

> EN — Nothing could be heard in that recording.

In quella registrazione non si è sentito nulla.

**`voice.errors.failed`**

> EN — Voice is unavailable right now. You can still type.

La voce non è disponibile in questo momento. Puoi sempre scrivere.
...
Non è stata inviata alcuna registrazione.
...
{credits, plural, one {# credito} other {# crediti}} al minuto, {minutes, plural, one {# minuto} other {# minuti}} al mese nel tuo piano.

**`voice.permission.editFirst`**

> EN — You read and correct the text before anything is sent.

Leggi e correggi il testo prima che venga inviato qualsiasi cosa.

**`voice.permission.notStored`**

> EN — The audio is sent for transcription and stored nowhere — not by us, not afterwards.

L'audio viene inviato per la trascrizione e non viene conservato da nessuna parte — né da noi né dopo.

**`voice.permission.pressToStart`**

> EN — Recording starts only when you press, and stops when you press again.

La registrazione parte solo quando premi e si ferma quando premi di nuovo.
...
Eliminare questa idea? L'operazione è irreversibile.
...
Potrebbe essere collegato a: {titles}. Vuoi collegarli?
...
Eliminare questo {label}? L'operazione è irreversibile.
...
Ha letto anche {count, plural, one {# messaggio precedente} other {# messaggi precedenti}} su questa voce
...
Fai una domanda su questa voce — non serve spiegare il contesto, l'IA lo ha già.

**`askAi.placeholder`**

> EN — Ask anything about this entry...

Chiedi qualsiasi cosa su questa voce...
...
La connessione si è interrotta prima che la risposta fosse completata.
...
A quale modulo vuoi collegarti?
