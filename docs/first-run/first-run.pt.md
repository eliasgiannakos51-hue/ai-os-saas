...
A pesquisa está indisponível neste momento — isto não é um resultado vazio. Tenta novamente daqui a pouco.
...
Exportar ou apagar a tua conta
...
Registos de sites, apps, imagens e vídeos
...
Nome próprio do assistente

**`pricing.rows.deepResearch`**

> EN — Deep Research runs / month

Execuções de Deep Research / mês
...
Mensagens de chat gratuitas / mês
...
A ligação caiu antes de a resposta terminar.

**`voice.errors.streamInterruptedPartial`**

> EN — The connection dropped — the reply above may be incomplete.

A ligação caiu — a resposta acima pode estar incompleta.
