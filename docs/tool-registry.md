...
|---|---|---|---|
| estimation profile key | how much to hold, in tokens | `ACTION_PROFILES` in `src/lib/billing/estimate.ts` | 26 keys |
| reserve action string | what the hold is labelled | 3rd argument of `reserveCredits` in `src/lib/billing/reservations.ts` | 26 distinct |
| settlement feature string | what the charge is labelled, and what the margin knob is named after | `feature` of `settleReservation` | 47 distinct |
| route path | what the browser calls | `src/app/api/**/route.ts` | 130 routes |
...
**23 of the 47 settlement features exist only at runtime.** They are built
...
**Zero of the 130 route files export a type or interface**, and there is no
