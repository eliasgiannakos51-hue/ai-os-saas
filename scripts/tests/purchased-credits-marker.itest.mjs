...
// THE LOCKDOWN, as this repository ships it: two dated migrations rather
// than one pasteable hotfix.
//
// The hotfix this test was written against listed the five credit
// functions by signature. That is what a pasteable emergency fix should
// do — but as a permanent mechanism it is the defect it was fixing: a
// per-file revoke list only covers the functions somebody remembered to
// add to it, so the next migration adds one and it ships executable by
// `authenticated`. 20260818000000_function_grants.sql instead loops over
// pg_proc and revokes on EVERY function in the schema, and is dated to run
// last so a function created by any earlier migration is covered by
// construction.
//
// The stale 8-argument overload is dropped by its own file for the same
// reason: it is a one-time correction with a date, not a rule.
const LOCKDOWN = [
