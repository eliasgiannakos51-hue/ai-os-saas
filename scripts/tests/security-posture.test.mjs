...
const NO_SESSION_BY_DESIGN = {
  ...
  "src/app/api/health/route.ts":
    "the uptime probe, and the one route whose intended caller IS a stranger: an external monitor has no session, and a health check behind auth monitors the auth rather than the app. WHAT AN ANONYMOUS CALLER LEARNS is a value from a closed vocabulary in lib/health/classify.ts and nothing else — no table name, no column, no provider message, no version, no counts, no host. The provider's real code and message are available only at ?verbose=1 with the CRON_SECRET bearer token, and even that path runs through scrubSecrets() first, because a message crossing from the log into an HTTP response is the one place a credential could ever escape. Its single query reads one column of one row of user_onboarding and DISCARDS the row; only `error` is read. user_onboarding rather than a recent table on purpose: the first version probed agent_templates, added by 20260826000000, and reported the database DOWN in production while the database was healthy and only that migration was unrun. It is not rate-limited and that is deliberate: checkRateLimit is itself a database round trip, so it would double the cost it bounds, and a monitor handed a 429 records an outage that did not happen. Instead the in-flight probe promise is shared, measured at one query for fifty simultaneous requests.",
