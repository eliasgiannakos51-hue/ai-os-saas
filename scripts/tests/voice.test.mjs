...
const {
  VOICE_RATES_USD, VOICE_MODELS, MAX_CLIP_SECONDS, MAX_SPEAK_CHARS,
  transcribeCostUsd, speakCostUsd, voiceCredits, creditsPerVoiceMinute,
  SPOKEN_CHARS_PER_MINUTE, DEFAULT_VOICE_MINUTE_LIMITS, VOICE_LIMIT_ENV_VARS,
  parseVoiceMinuteLimits, voiceMinutesForPlan, minutesToSeconds, secondsToMinutes,
  speakCharsToSeconds, voiceAllowance, fitsInAllowance,
} = pricing;
const {
  AUDIO_CONSTRAINTS, PREFERRED_MIME_TYPES, ACCEPTED_AUDIO_TYPES, isAcceptedAudioType,
  MAX_AUDIO_BYTES, VOICE_LANGUAGES, languageHint, SILENCE, frameRms, turnShouldEnd,
  PLAYBACK_RATES, VOICES, DEFAULT_VOICE, speakableWords, wordIndexAt,
} = config;
const {
  VOICE_STATES, SMOOTHING, smoothAmplitude, PULSE_RANGE, pulseScale, HALO_OPACITY,
  haloOpacity, ROTATION_SECONDS_PER_TURN, rotationDegrees, orbFrame, staticFrame,
  STATE_COLOR_VAR, isMicrophoneOpen,
} = visual;
...
ok("transcribe is priced per MINUTE and speak per CHARACTER (the two providers' own units)",
  VOICE_RATES_USD.transcribePerMinute > 0 && VOICE_RATES_USD.speakPer1kChars > 0);

ok("a 60s clip costs exactly one minute of transcription",
  Math.abs(transcribeCostUsd(60) - VOICE_RATES_USD.transcribePerMinute) < 1e-12,
  `${transcribeCostUsd(60)}`);
ok("a 30s clip costs half a minute — seconds are NOT rounded up to a minute",
  Math.abs(transcribeCostUsd(30) - VOICE_RATES_USD.transcribePerMinute / 2) < 1e-12);
ok("1,000 characters costs exactly the per-1k rate",
...
for (const bad of [-1, -1e9, NaN, Infinity, -Infinity]) {
  ok(`transcribeCostUsd(${bad}) is never negative`, transcribeCostUsd(bad) >= 0, `${transcribeCostUsd(bad)}`);
  ok(`speakCostUsd(${bad}) is never negative`, speakCostUsd(bad) >= 0, `${speakCostUsd(bad)}`);
}

ok("MAX_CLIP_SECONDS is a real ceiling, not a formality", MAX_CLIP_SECONDS > 0 && MAX_CLIP_SECONDS <= 300);
ok("MAX_AUDIO_BYTES fits a full-length clip at a generous bitrate",
  MAX_AUDIO_BYTES >= (MAX_CLIP_SECONDS * 32_000) / 8,
  `${MAX_AUDIO_BYTES} bytes vs ${(MAX_CLIP_SECONDS * 32_000) / 8} needed`);

ok("speakCharsToSeconds and SPOKEN_CHARS_PER_MINUTE agree",
  speakCharsToSeconds(SPOKEN_CHARS_PER_MINUTE) === 60,
  `${speakCharsToSeconds(SPOKEN_CHARS_PER_MINUTE)}`);
ok("minutesToSeconds/secondsToMinutes round-trip whole minutes",
...
for (const margin of MARGINS) {
  for (const creditPriceEur of CREDIT_PRICES) {
    for (const usdToEurRate of USD_RATES) {
      const cfg = { ...pricingConfig.resolvePricingConfig(), creditPriceEur, usdToEurRate };
      for (const seconds of CLIP_SECONDS) {
        ...
        if (ratio < margin) {
          ok(`margin holds: transcribe ${seconds}s @ M=${margin}`, false,
      ...
      for (const characters of SPEAK_CHARS) {
        ...
        if (ratio < margin) {
          ok(`margin holds: speak ${characters} chars @ M=${margin}`, false,
            `ratio ${ratio.toFixed(3)} < ${margin} (${credits} credits, cost €${costEur})`);
        }
      }
    }
...
ok(`every one of ${sweep} priced voice calls clears its own margin (worst ${worstRatio.toFixed(2)}x)`,
  worstRatio >= MARGIN_MULTIPLIER_MIN, worstCase ?? "");
...
ok("no billable clip length is charged zero credits", zeroCharges === 0, `${zeroCharges} lengths charged nothing`);

ok("voice is on the margin policy's feature map (so a per-plan override reaches it)",
...
ok(`speech costs far more per minute than listening (${perMinute.transcribe} vs ${perMinute.speak} credits at M=4)`,
...
ok("orbFrame returns EXACTLY scale, halo and rotation — nothing that would force layout or paint",
...
const orbBlocks = [...css.matchAll(/\.voice-orb[^{]*\{([^}]*)\}/g)].map((m) => m[1]);
ok("the .voice-orb rules exist in globals.css", orbBlocks.length > 0, `${orbBlocks.length} blocks`);
const forbidden = ["width:", "height:", "top:", "left:", "margin", "padding", "box-shadow:", "filter:", "font-size:"];
const animatedProps = orbBlocks
  .flatMap((block) => [...block.matchAll(/transition\s*:([^;]*);|animation\s*:([^;]*);/g)])
  .map((m) => (m[1] ?? m[2]).trim());
const badTransition = animatedProps.filter((decl) =>
  forbidden.some((p) => decl.includes(p.replace(":", "")))
);
ok("no .voice-orb transition or animation names a property outside the compositor",
  badTransition.length === 0, JSON.stringify(badTransition));
const varDriven = orbBlocks.filter((b) => b.includes("--orb-scale") || b.includes("--orb-halo") || b.includes("--orb-rotate"));
for (const block of varDriven) {
  const usesVarOutsideSafe = [...block.matchAll(/([a-z-]+)\s*:\s*[^;]*var\(--orb-(?:scale|halo|rotate)\)/g)]
    .map((m) => m[1])
    .filter((prop) => prop !== "transform" && prop !== "opacity");
  ok(`--orb-* is consumed only by transform/opacity (${usesVarOutsideSafe.join(",") || "clean"})`,
    usesVarOutsideSafe.length === 0);
}

ok("silence produces scale exactly 1 — the globe does not visibly shrink when somebody stops talking",
  pulseScale("listening", 0) === 1);
ok("a full-amplitude frame reaches the state's ceiling and no further",
  pulseScale("listening", 1) === PULSE_RANGE.listening.max && pulseScale("listening", 5) === PULSE_RANGE.listening.max);
ok("thinking does not pulse at all — it rotates",
  pulseScale("thinking", 1) === 1 && ROTATION_SECONDS_PER_TURN.thinking > 0);
ok("idle does not move on any axis",
  pulseScale("idle", 1) === 1 && rotationDegrees("idle", 999_999) === 0 && haloOpacity("idle", 1) === 0);
ok("speaking pulses to a DIFFERENT ceiling and a DIFFERENT colour from listening",
  PULSE_RANGE.speaking.max !== PULSE_RANGE.listening.max &&
  STATE_COLOR_VAR.speaking !== STATE_COLOR_VAR.listening);

ok("every state has its own colour variable", new Set(Object.values(STATE_COLOR_VAR)).size === VOICE_STATES.length);
...
ok("a [data-theme=\"light\"] block actually defines voice colours",
  lightVoiceBlock.length > 0, `${lightBlocks.length} light blocks, none with --voice-`);
...
for (const state of VOICE_STATES) {
  ...
  ok(`${varName} is redefined for the light theme`, declared.test(lightVoiceBlock));
  ...
  const darkValue = darkVoiceBlock.match(new RegExp(`${varName}\\s*:\\s*([^;]+);`))?.[1]?.trim();
  const lightValue = lightVoiceBlock.match(new RegExp(`${varName}\\s*:\\s*([^;]+);`))?.[1]?.trim();
  ok(`${varName} is a DIFFERENT colour in the light theme, not the dark one copied across`,
    Boolean(darkValue) && Boolean(lightValue) && darkValue !== lightValue,
    `dark ${darkValue} / light ${lightValue}`);
}

ok("rotation never exceeds a full turn (it is fed straight into rotate())",
  [0, 1, 999, 18_000, 1e9].every((ms) => rotationDegrees("thinking", ms) >= 0 && rotationDegrees("thinking", ms) < 360));

// Reduced motion: still, but still LEGIBLE.
ok("staticFrame does not move", VOICE_STATES.every((s) => staticFrame(s).scale === 1 && staticFrame(s).rotation === 0));
ok("staticFrame keeps listening visually distinct from idle without moving",
...
const appSwitchRules = [...flatCss.matchAll(/html\[data-motion="reduce"\][^{]*\{[^}]*\}/g)].map((m) => m[0]);
const orbAppSwitchRule = appSwitchRules.find((r) => namesAllParts(r));
ok("the app's own motion switch kills the orb's movement, on every moving part",
  Boolean(orbAppSwitchRule) &&
  /transform:\s*none/.test(orbAppSwitchRule) && /animation:\s*none/.test(orbAppSwitchRule),
  orbAppSwitchRule?.slice(0, 140) ?? `${appSwitchRules.length} data-motion rules, none naming every orb part`);

const reducedMotionBlocks = cssBlocks(css, "@media (prefers-reduced-motion: reduce)");
const orbUnderOsSetting = reducedMotionBlocks.filter((b) => namesAllParts(b));
ok("the OS setting kills it too — the two switches are not the same switch",
...
ok("...and an explicit 'full motion' choice still wins over the OS default",
  orbUnderOsSetting.some((b) => b.includes('html:not([data-motion="full"])')));
ok("...and what those rules kill is transform, transition and animation — the three things that move",
  orbUnderOsSetting.some((b) =>
    /transform:\s*none/.test(b) && /transition:\s*none/.test(b) && /animation:\s*none/.test(b)));
ok("what those rules kill is movement, not the globe itself — reduced motion still shows which state it is in",
  orbUnderOsSetting.every((b) => !/display:\s*none|visibility:\s*hidden|opacity:\s*0\s*!/.test(b)));
...
ok("a loud frame after silence moves most of the way immediately",
  smoothAmplitude(0, 1) === SMOOTHING.attack);
ok("a silent frame after a loud one settles slowly rather than snapping shut",
  smoothAmplitude(1, 0) > 0.8, `${smoothAmplitude(1, 0)}`);
ok("smoothing never leaves 0..1 however it is fed",
  [[NaN, NaN], [Infinity, 5], [-3, -3], [0.5, 2]].every(([p, t]) => {
    const v = smoothAmplitude(p, t);
    return Number.isFinite(v) && v >= -1 && v <= 1;
  }));

ok("the microphone is open in exactly ONE state, and it is the one called listening",
...
const loudFrame = Uint8Array.from({ length: 256 }, (_, i) => (i % 2 ? 200 : 56));
ok("a loud frame is above the threshold", frameRms(loudFrame) > SILENCE.rmsThreshold, `${frameRms(loudFrame)}`);
const roomToneFrame = Uint8Array.from({ length: 256 }, (_, i) => 128 + (i % 3) - 1);
ok("room tone stays BELOW the threshold (a quiet room is not zero)",
  frameRms(roomToneFrame) < SILENCE.rmsThreshold, `${frameRms(roomToneFrame)}`);
ok("frameRms survives an empty frame", frameRms(new Uint8Array(0)) === 0);

ok("a hesitation right after pressing does not end the turn",
  turnShouldEnd({ elapsedMs: 300, quietMs: 300, heardSpeech: false }).end === false);
ok("silence before ANY speech never ends the turn as 'silence'",
  turnShouldEnd({ elapsedMs: 5000, quietMs: 5000, heardSpeech: false }).end === false);
ok("a full hangover after speech ends the turn, and says why",
  JSON.stringify(turnShouldEnd({ elapsedMs: 5000, quietMs: SILENCE.hangoverMs, heardSpeech: true })) ===
    JSON.stringify({ end: true, reason: "silence" }));
ok("a pause SHORTER than the hangover does not cut somebody off mid-sentence",
  turnShouldEnd({ elapsedMs: 5000, quietMs: SILENCE.hangoverMs - 1, heardSpeech: true }).end === false);
ok("the turn ceiling ends it regardless, and distinguishes 'you stopped' from 'I heard nothing'",
  turnShouldEnd({ elapsedMs: SILENCE.maxTurnMs, quietMs: 0, heardSpeech: true }).reason === "max_turn" &&
  turnShouldEnd({ elapsedMs: SILENCE.maxTurnMs, quietMs: 0, heardSpeech: false }).reason === "no_speech");
ok("the hangover is longer than a pause inside a sentence and shorter than one after it",
  SILENCE.hangoverMs >= 800 && SILENCE.hangoverMs <= 2000);
ok("the conversational ceiling is well under the hard clip ceiling",
...
const recorderSrc = readFileSync("src/components/voice/use-recorder.ts", "utf8");
const getUserMediaCalls = [...recorderSrc.matchAll(/getUserMedia\s*\(/g)].length;
ok("getUserMedia is reached from exactly one place in the whole recorder",
  getUserMediaCalls === 1, `${getUserMediaCalls} call sites`);
...
for (let from = 0; ; ) {
  ...
  if (at < 0) break;
  const body = blockAfter(recorderSrc, recorderSrc.slice(at, at + "useEffect(".length));
  if (body) effectBodies.push(body);
  from = at + 1;
}
ok(`the recorder's useEffect calls were found (${effectBodies.length})`, effectBodies.length > 0);
ok("no useEffect in the recorder starts a recording — the microphone opens on a press and on nothing else",
  effectBodies.every((body) => !/\bstart\(\)/.test(body)),
  effectBodies.filter((b) => /\bstart\(\)/.test(b)).map((b) => b.slice(0, 60)).join(" | "));
ok("the recorder stops every TRACK, not just the MediaRecorder (the browser's dot must go out)",
...
ok("the STOP path releases the stream — not only the error path",
  (onStopBody ?? "").includes("releaseStream("),
  (onStopBody ?? "").slice(0, 80));
ok("and releases it BEFORE handing the audio on, so the browser's dot goes out when the UI says it did",
  (onStopBody ?? "").indexOf("releaseStream(") >= 0 &&
  (onStopBody ?? "").indexOf("releaseStream(") < (onStopBody ?? "").indexOf("onResult?.("),
  `release at ${(onStopBody ?? "").indexOf("releaseStream(")}, callback at ${(onStopBody ?? "").indexOf("onResult?.(")}`);
ok("...and not deferred past it with a microtask or a timer",
  !/queueMicrotask|setTimeout/.test((onStopBody ?? "")));
ok("a hard clip ceiling is armed on every start",
...
const anySrc = voiceSources.map((f) => stripComments(readFileSync(f, "utf8"))).join("\n");
ok("nothing in the voice components writes the audio anywhere persistent",
  !/localStorage|sessionStorage|indexedDB|\.upload\(/i.test(anySrc),
  (anySrc.match(/localStorage|sessionStorage|indexedDB|\.upload\(/gi) ?? []).join(","));
ok("...and the raw text still MENTIONS localStorage, so the strip above is doing work rather than matching nothing",
  voiceSources.some((f) => readFileSync(f, "utf8").includes("localStorage")));
ok("every blob URL created in the voice components is revoked somewhere",
  (anySrc.match(/createObjectURL/g) ?? []).length <= (anySrc.match(/revokeObjectURL/g) ?? []).length,
...
ok("the mic button renders NOTHING when transcription is unavailable (rather than failing on press)",
...
ok("the explanation is gated on a real 'have they seen it' flag, not on a constant",
  (pressBody ?? "").includes("explainedRef.current") && (pressBody ?? "").includes("setExplaining(true)"),
  (pressBody ?? "").slice(0, 120));
ok("the first press cannot reach getUserMedia — it returns after showing the explanation",
  /if \(!explainedRef\.current\)\s*\{[^}]*setExplaining\(true\);[^}]*return;/s.test(pressBody ?? ""));
...
ok("the transcript is put into a DRAFT by the upload path, never handed to the parent from it",
  (sendBody ?? "").includes("setDraft(") && !(sendBody ?? "").includes("onTranscript("),
  (sendBody ?? "").split("\n").filter((l) => l.includes("onTranscript")).join(" | "));
ok("onTranscript is called from exactly one place in the whole component — the accept button",
  (inputSrc.match(/onTranscript\(/g) ?? []).length === 1,
  `${(inputSrc.match(/onTranscript\(/g) ?? []).length} call sites`);
ok("the draft dialog says out loud that nothing has been sent", inputSrc.includes("draft.notSent"));
...
// takes.
const constantGuards = voiceSources
  .flatMap((f) => {
    const src = stripComments(readFileSync(f, "utf8"));
    return [...src.matchAll(/if \(\s*(?:true|false)\s*\)/g)].map(() => f);
  });
ok("no voice component guards a promise on a constant", constantGuards.length === 0, constantGuards.join(", "));

const playerSrc = readFileSync("src/components/voice/voice-player.tsx", "utf8");
ok("the Listen button carries its PRICE before it is pressed",
  playerSrc.includes('t("listenFor"'));
ok("the player renders nothing when speech is unavailable",
...
const settingsSrc = readFileSync("src/components/settings/voice-settings.tsx", "utf8");
ok("the settings panel distinguishes 'your plan does not include it' from 'this deployment has no keys'",
  settingsSrc.includes('t("notIncluded")') && settingsSrc.includes('t("notConfigured")'));
ok("...and only hides itself while the status call is still in flight",
  /if \(!v\.loaded\) return null;/.test(settingsSrc) &&
  (settingsSrc.match(/return null;/g) ?? []).length === 1);
ok("it shows BOTH prices, because they are nowhere near each other",
  settingsSrc.includes("creditsPerMinute.transcribe") && settingsSrc.includes("creditsPerMinute.speak"));
ok("it repeats the privacy sentence where somebody would go looking for it",
  settingsSrc.includes('t("privacy")'));
ok("the settings page mounts it", readFileSync("src/app/dashboard/settings/page.tsx", "utf8").includes("<VoiceSettings />"));

const conversationSrc = readFileSync("src/components/voice/voice-conversation.tsx", "utf8");
...
ok("the conversation loop returns to listening when the reply finishes",
  (onEndedBody ?? "").includes("listen()"),
  (onEndedBody ?? "").slice(0, 100));
ok("...and the only thing that can stop it is the session having been abandoned",
  /^\{\s*if \(abandonedRef\.current\) return;/.test((onEndedBody ?? "").replace(/\n\s*/g, "\n").trim().replace(/\n/g, " ").replace(/\{ /, "{ ")) ||
  /if \(abandonedRef\.current\) return;/.test(onEndedBody ?? ""),
  (onEndedBody ?? "").slice(0, 120));
ok("no early return in onended is unconditional",
  !/\(\s*(?:true|false)\s*\)\s*return/.test(onEndedBody ?? ""));
ok("tapping while it speaks interrupts it",
  conversationSrc.includes('state === "speaking"') && conversationSrc.includes("conversation.interrupt"));
ok("the transcript of every turn is rendered as text as well as spoken",
  conversationSrc.includes("turns.map("));
ok("an abandoned session is checked after every await",
...
ok("every plan has a minute limit", PLANS.every((p) => typeof DEFAULT_VOICE_MINUTE_LIMITS[p.slug] === "number"));
ok("every plan has an env var name", PLANS.every((p) => typeof VOICE_LIMIT_ENV_VARS[p.slug] === "string"));
ok("the env var names are unique", new Set(Object.values(VOICE_LIMIT_ENV_VARS)).size === PLANS.length);
ok("the free plan gets no voice minutes by default (the providers bill per minute)",
  DEFAULT_VOICE_MINUTE_LIMITS.free === 0);
const paidSlugs = PLANS.map((p) => p.slug).filter((s) => s !== "free");
ok("every paid plan gets some", paidSlugs.every((s) => DEFAULT_VOICE_MINUTE_LIMITS[s] > 0));

const parsed = parseVoiceMinuteLimits({ VOICE_MINUTES_STARTER: "45" });
ok("a valid override is taken", parsed.limits.starter === 45);
ok("an untouched plan keeps its default", parsed.limits.growth === DEFAULT_VOICE_MINUTE_LIMITS.growth);
for (const bad of ["-1", "abc", "1.5", "1e6", "999999"]) {
  const r = parseVoiceMinuteLimits({ VOICE_MINUTES_STARTER: bad });
  ok(`"${bad}" is rejected and WARNED about, not silently taken`,
    r.limits.starter === DEFAULT_VOICE_MINUTE_LIMITS.starter && r.warnings.length === 1,
    JSON.stringify(r));
}
...
for (const blank of ["", "   ", "\t"]) {
  ...
  ok(`${JSON.stringify(blank)} is treated as unset: default kept, nothing warned`,
    r.limits.starter === DEFAULT_VOICE_MINUTE_LIMITS.starter && r.warnings.length === 0,
    JSON.stringify(r));
}
ok("a warning names the plan, the env var and the offending value — enough to act on without reading the code",
  (() => {
    const w = parseVoiceMinuteLimits({ VOICE_MINUTES_GROWTH: "-5" }).warnings[0];
    return w?.plan === "growth" && w?.envVar === "VOICE_MINUTES_GROWTH" && w?.value === "-5" && typeof w?.reason === "string";
  })());
ok('"0" is a valid override that turns voice off for a plan',
  parseVoiceMinuteLimits({ VOICE_MINUTES_STARTER: "0" }).limits.starter === 0);
ok("an unknown plan slug falls back rather than throwing", voiceMinutesForPlan("nonsense") >= 0);

const allowance = voiceAllowance(30, 600);
ok("an allowance reports what is left, in seconds",
...
ok("a clip that would cross the ceiling is refused", fitsInAllowance(allowance, 30 * 60) === false);
ok("an exhausted allowance never goes negative", voiceAllowance(30, 99_999).remainingSeconds === 0);
ok("a zero limit refuses everything", fitsInAllowance(voiceAllowance(0, 0), 1) === false);
...
const localeList = [...i18nConstants.matchAll(/"([a-z]{2})",/g)].map((m) => m[1]);
ok(`voice speaks every locale the product is translated into (${VOICE_LANGUAGES.length})`,
  VOICE_LANGUAGES.length === 10, VOICE_LANGUAGES.join(","));
ok("the language list IS the locale list, not a second copy that can drift",
  VOICE_LANGUAGES.every((l) => localeList.includes(l)));
ok("an unknown locale sends NO hint rather than a wrong one", languageHint("kl") === undefined);
ok("a known locale sends itself", languageHint("el") === "el");
ok("a null locale sends no hint", languageHint(null) === undefined && languageHint(undefined) === undefined);

ok("mono 16kHz is asked for (Whisper resamples anyway; 48k stereo is 3x the upload)",
  AUDIO_CONSTRAINTS.channelCount === 1 && AUDIO_CONSTRAINTS.sampleRate === 16_000);
ok("echo cancellation, noise suppression and gain control are all on",
  AUDIO_CONSTRAINTS.echoCancellation && AUDIO_CONSTRAINTS.noiseSuppression && AUDIO_CONSTRAINTS.autoGainControl);
ok("Safari's MP4 is in the preferred list (a WebM-only list means no iOS)",
  PREFERRED_MIME_TYPES.some((t) => t.startsWith("audio/mp4")));
ok("every preferred container is one the route will accept",
  PREFERRED_MIME_TYPES.every((t) => isAcceptedAudioType(t)),
  PREFERRED_MIME_TYPES.filter((t) => !isAcceptedAudioType(t)).join(","));
ok("a codec parameter does not defeat the type check", isAcceptedAudioType("audio/webm;codecs=opus"));
...
ok("a missing type is refused", isAcceptedAudioType(null) === false && isAcceptedAudioType("") === false);

ok("1x is offered and is not at either end of the speed list",
  PLAYBACK_RATES.includes(1) && PLAYBACK_RATES[0] < 1 && PLAYBACK_RATES[PLAYBACK_RATES.length - 1] > 1);
ok("every voice has a distinct id", new Set(VOICES.map((v) => v.id)).size === VOICES.length);
ok("the default voice is one of them", VOICES.some((v) => v.key === DEFAULT_VOICE));

const words = speakableWords("Hello there, world.");
ok("word offsets point back into the ORIGINAL string",
  words.every((w) => "Hello there, world.".slice(w.start, w.end) === w.word),
  JSON.stringify(words));
ok("nothing is highlighted at rest", wordIndexAt(words, 0, 0) === -1);
ok("the first word is highlighted at the start", wordIndexAt(words, 1, 10_000) === 0);
ok("the last word is highlighted just before the end",
...
ok("nothing is highlighted once the clip has finished",
  wordIndexAt(words, 10_000, 10_000) === -1 && wordIndexAt(words, 20_000, 10_000) === -1);
ok("the highlight walks forward monotonically through the clip",
  (() => {
    let previous = -1;
...
ok("an empty string produces no words and no crash", speakableWords("").length === 0 && wordIndexAt([], 5, 10) === -1);
...
ok(`the voice namespace exists and is not a stub (${enVoiceKeys.length} keys)`, enVoiceKeys.length >= 40);
...
ok("every voice key exists in every one of the ten locales", missing.length === 0, missing.slice(0, 10).join(", "));
...
for (const locale of LOCALES) {
  ...
  ok(`${locale}: every VoiceState has a label`, VOICE_STATES.every((s) => typeof v?.states?.[s] === "string"));
  ok(`${locale}: every voice has a name`, VOICES.every((x) => typeof v?.voices?.[x.key] === "string"));
}
...
const routeSrc = ["transcribe", "speak", "usage"]
  ...
  .join("\n");
const emittedCodes = new Set([...routeSrc.matchAll(/code:\s*"([a-z_]+)"/g)].map((m) => m[1]));
// The provider failure kinds arrive as `code: result.failure.kind`.
for (const kind of [...readFileSync("src/lib/voice/voice-providers.ts", "utf8").matchAll(/kind:\s*"([a-z_]+)"/g)].map((m) => m[1])) {
  emittedCodes.add(kind);
}
// And the recorder's own client-side reasons.
for (const reason of ["denied", "unsupported", "no_speech", "failed"]) emittedCodes.add(reason);
ok(`the routes and the recorder between them emit ${emittedCodes.size} distinct error codes`, emittedCodes.size >= 12);
let codeGaps = [];
for (const locale of LOCALES) {
  for (const code of emittedCodes) {
    if (typeof messages[locale].voice?.errors?.[code] !== "string") codeGaps.push(`${locale}:${code}`);
  }
}
ok("every error code a user can hit has a translation in every locale",
  codeGaps.length === 0, codeGaps.slice(0, 12).join(", "));

// The ICU placeholders must survive translation — {credits} rendered as
// a literal is a price nobody can read.
const PLACEHOLDERS = { listenFor: ["credits"], costPerMinute: ["credits"], "permission.cost": ["credits", "minutes"] };
let placeholderGaps = [];
for (const locale of LOCALES) {
  for (const [key, names] of Object.entries(PLACEHOLDERS)) {
    const value = key.split(".").reduce((a, k) => a?.[k], messages[locale].voice);
    for (const name of names) {
      if (typeof value !== "string" || !value.includes(`{${name}}`)) placeholderGaps.push(`${locale}:${key}:{${name}}`);
    }
  }
}
ok("every priced string keeps its {credits}/{minutes} placeholder in every locale",
  placeholderGaps.length === 0, placeholderGaps.join(", "));
...
for (const key of ["OPENAI_API_KEY", "ELEVENLABS_API_KEY"]) {
  ok(`${key} is read from process.env by name`, providersSrc.includes(`process.env.${key}`));
  const envAt = providersSrc.indexOf(`process.env.${key}`);
  const fetchAt = providersSrc.indexOf("fetch(");
  ok(`${key} is checked before the first network call`, envAt >= 0 && envAt < fetchAt);
}
ok("a missing key is its own outcome, not an error",
  providersSrc.includes('kind: "not_configured"'));
ok("an empty transcript is its own outcome, not an error",
...
ok(`every provider fetch is wrapped in withTimeout (${timedFetches}/${providerFetches})`,
  providersSrc.includes("AbortController") && providerFetches > 0 && timedFetches >= providerFetches);
ok("the timeout is a real one, not a placeholder",
  /PROVIDER_TIMEOUT_MS\s*=\s*[\d_]{5,}/.test(providersSrc),
  providersSrc.split("\n").find((l) => l.includes("PROVIDER_TIMEOUT_MS =")) ?? "not found");
...
ok("the language field is appended ONLY when a hint exists — never a default that constrains the detector",
  /if \(params\.languageHint\)[^\n]*append\("language"/.test(providersSrc),
  providersSrc.split("\n").filter((l) => l.includes('"language"')).join(" | "));
ok("...and languageHint itself refuses to invent one for an unknown locale",
