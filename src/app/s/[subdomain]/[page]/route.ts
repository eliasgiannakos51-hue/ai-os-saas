...
import { validatePageSlug, normalisePages } from "@/lib/publishing/website-pages";
import {
...
export async function GET(request: Request, { params }: { params: { subdomain: string; page: string } }) {
  try {
    ...
    const { data: site, error } = await admin
      ...
      .select("id, user_id, html_content, pages, status, is_active, updated_at")
      .eq("subdomain", check.subdomain)
      .maybeSingle();

    if (error) {
      logApiError("/s/[subdomain]/[page]", error, { stage: "load_site" });
    ...
    // THE SLUG IS VALIDATED THE SAME WAY THE SUBDOMAIN IS, and before it
    // is compared against anything. A request for /s/x/../admin never
    // reaches a lookup: the shape check rejects it, which is the only
    // defence that survives encoding — a rule written as "does not
    // contain .." falls to %2e%2e, and one written on the decoded string
    // falls to double encoding.
    const slug = validatePageSlug(params.page);
    if (!slug.ok) return notFoundResponse();

    const { pages } = normalisePages(site.pages);
    const page = pages.find((p) => p.slug === slug.slug);
    // A SITE WITH NO SUCH PAGE IS A 404, not a redirect to the home page.
    // Serving home for an unknown path would tell a crawler that every
    // URL under this site exists, and tell a visitor who mistyped that
    // they arrived.
    ...
    logApiError("/s/[subdomain]/[page]", err);
    return new Response(SERVICE_UNAVAILABLE_HTML, {
      status: 503,
      headers: { ...notFoundHeaders(), "Retry-After": "30" },
    });
...
async function recordView(
  ...
  try {
    ...
    if (error) logApiError("/s/[subdomain]/[page]", error, { stage: "record_view" });
  } catch (err) {
    logApiError("/s/[subdomain]/[page]", err, { stage: "record_view_unhandled" });
  }
}
