...
export async function runAgentTask(params: {
  ...
  // BOTH: routed through the provider layer, AND checked for truncation.
  //
  // The routing came from the multi-provider work; the truncation guard
  // came from the verification work. Taking either alone loses something
  // real — without routing there is one vendor and no failover, and
  // without the guard a run that hit its token ceiling is emailed to the
  // user as the agent's finished output. A scheduled agent quietly
  // reporting half an answer every morning is the failure that guard
  // exists for, and it does not stop mattering because the call now goes
  // through an adapter.
  const generated = modelTextFrom(outcome);
