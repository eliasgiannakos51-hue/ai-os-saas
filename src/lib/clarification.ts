...
CRITICAL REAL FACTS: separately from the above, if actually fulfilling this request would obviously require specific real-world facts that are NOT given — exact prices, a real address, a phone number, exact opening/business hours, or the name of a specific product/service being described — treat that as needing clarification too, even if the request otherwise reads as clear. Ask directly for exactly the missing fact(s), IN THE SAME LANGUAGE THE DESCRIPTION IS WRITTEN IN — these questions are shown to the person who wrote it. In English they would read "What are your prices/menu items?" and "What's the business address and phone number?"; for a description written in Greek, ask them in Greek. Do NOT ask for facts that don't matter for a reasonable, generic-but-usable result (e.g. an exact tagline, a specific font, or minor content the request doesn't imply is essential) — this is only for facts a real person would need to know before the output could be considered accurate rather than made up.`;

/**
 * EVERY QUESTION THIS FILE PRODUCES IS SHOWN TO THE PERSON WHO WROTE THE
 * DESCRIPTION, and until 2026-09-06 nothing here said what language to
 * ask in. Not one mention, in a file whose entire output is user-facing
 * text — while website-builder.ts, which emits HTML, names the language
 * eight times.
 *
 * WHAT MADE IT INVISIBLE: this runs on Sonnet 4.6, which infers the
 * language from the description sitting in the same prompt and asks in
 * Greek anyway. That is the same thing that hid the English disclaimer in
 * AI_SAFETY_BOUNDARIES_EN — Sonnet and Opus translated it, Haiku pasted
 * it — and it is why "it works when I try it" is not evidence here. The
 * instruction is cheap; depending on inference is not.
 */
const ASK_IN_THE_USERS_LANGUAGE =
