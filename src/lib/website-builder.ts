...
- WRITING A GOOD data-image-query (this decides whether the photo actually matches the site):
  ...
  * Name the SUBJECT and the STYLE/SETTING: "greek mezze plates taverna table". NEVER the trading name — "Taverna Nikos" is not in any photo library and returns nothing.
  * THE PLACE. If the brief names a city, island, region or neighbourhood, that place belongs in the queries. Which END it goes on is a real decision: a query that returns nothing is retried with its TRAILING words dropped, so a word at the front survives every retry and a word at the back is the first thing lost.
    - ESTABLISHING shots — the hero, an exterior, a view, the street, anything whose job is "this is where we are": put the PLACE FIRST, e.g. "Santorini Greece caldera village whitewashed" or "Thessaloniki Ladadika street evening". If the exact shot is not there, the retry still returns that place.
    - DETAIL shots — a dish, a product, a treatment room, a close-up of the work: put the SUBJECT FIRST and the place LAST, e.g. "grilled octopus mezze plate Santorini". If it misses, the retry drops the place and still returns the right subject rather than a postcard.
  * IF THE BRIEF ASKS FOR PHOTOS OF A PLACE — "βάλε φωτογραφίες από τη Σαντορίνη", "use photos from this area" — that is a REQUIREMENT, not a preference. Every query must carry the place name and the hero must be an establishing shot with the place first. A generic industry photo is not an acceptable substitute, and neither is a photo of a different place.
...
1. THE PLACE — if the brief names a city, neighbourhood, region or country, search it together with the business type (e.g. "cafés Thessaloniki Ladadika neighbourhood", "Thessaloniki city centre character"). You are looking for real, checkable specifics you can write into the copy: the actual character of the area, real landmarks and districts nearby, what locals and visitors actually come there for, seasonality. A page that could be about any city in the world has failed this step.
   Search what the place LOOKS like as well as what it is: that is what makes a photo query findable. "Santorini" is a name; "whitewashed cave houses, blue domes, caldera cliffs" are the visual specifics that turn a query into the right photograph. Put two or three of them into the data-image-query values (see IMAGES).
