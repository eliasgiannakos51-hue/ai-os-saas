...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'en', 'How do I change plan?', 'Settings > Billing. Pick the plan you want and the change takes effect immediately. On an upgrade you pay only the difference for the rest of the period; on a downgrade you keep your current plan until the period you have already paid for ends.', 'billing', 1, true, array['change plan', 'upgrade', 'downgrade', 'switch plan', 'change subscription', 'move to a bigger plan']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('invoices', 'en', 'Where do I find my receipts?', 'Settings > Billing > Payment history. Every charge has a PDF receipt you can download from there. Payments run through Stripe and we never store your card details.', 'billing', 3, true, array['receipt', 'receipts', 'invoice', 'invoices', 'billing history', 'payment history']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('publish-website', 'en', 'How do I publish my website?', 'In Website Builder, open the site and press Publish. You choose a name for the address and the site goes live immediately. Every publish is kept as a version, so you can roll back to an earlier one if you need to.', 'websites', 1, true, array['publish', 'how do i publish', 'put it live', 'go live', 'make my site public']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'en', 'How do I make a mission?', 'In Mission Control you write a goal the way you would say it to a person — "I want more customers by spring". Ionexa breaks it into concrete steps, and you can then work through them yourself or hand some of them to an agent.', 'missions', 0, true, array['make a mission', 'mission control', 'goal', 'goals', 'create mission', 'missions']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('invite-code', 'en', 'Where do I enter an invite code?', 'There is a field for an invite code on the sign-up form. If you already have an account you enter it in Settings > Account. Once it is accepted, whatever it grants is active on your account straight away.', 'account', 0, true, array['invite code', 'beta code', 'where do i put the code', 'invitation', 'promo code']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('upload-files', 'en', 'Can I upload files?', 'Yes — PDF, Word, Excel, CSV, text and Markdown. You upload them in Files, Ionexa reads what is in them, and then you can ask questions about them. Your files are private: nobody else has access and every download uses a temporary link.', 'files', 0, true, array['upload a file', 'upload', 'pdf', 'files', 'documents', 'can i upload']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'en', 'How do I connect Gmail or Google Drive?', 'Settings > Integrations. Pick the service and approve access in Google''s own window. You see exactly what it will read before you approve, and you can disconnect whenever you like — the access keys are deleted immediately when you do.', 'integrations', 0, true, array['gmail', 'google drive', 'connect', 'integration', 'slack', 'how do i connect', 'link my email']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('languages', 'en', 'What languages does it work in?', 'The interface is available in ten languages and you change it in Settings. The AI replies in whatever language you write to it in, regardless of the interface language.', 'getting-started', 0, true, array['language', 'languages', 'english', 'greek', 'translation', 'change language']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'en', 'How do I delete my account?', 'Settings > Account > Delete account. You will be asked to confirm, and then everything goes: conversations, files, websites, agents, history. It cannot be undone. If you want a copy first, download your data from the same page.', 'privacy', 0, true, array['delete account', 'delete my account', 'close my account', 'remove my account']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('data-privacy', 'en', 'What do you do with my data?', 'Your data is yours. We do not sell it, we do not use it to train models, and every account sees only its own — that is enforced in the database, not just in the application. You can download it or delete it whenever you want.', 'privacy', 2, true, array['my data', 'privacy', 'security', 'do you sell my data', 'train models', 'is my data safe']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('team-members', 'en', 'Can I add people from my team?', 'Yes, on the plans that include a team. Settings > Team, invite by email and that person joins the same workspace. Which plans include it and how many seats they give is on /pricing.', 'account', 2, true, array['team', 'colleagues', 'invite a member', 'collaboration', 'add users', 'share my workspace']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('notifications', 'en', 'How do I set up notifications?', 'Settings > Notifications. Separately for email and for phone notifications, and per type — agent results, reminders, low credits. Critical security notifications cannot be switched off.', 'account', 3, true, array['notifications', 'email notifications', 'push', 'stop emailing me', 'turn off notifications']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'en', 'What is Ionexa?', 'A workspace where the AI does the work rather than only advising you: it makes your website, runs agents on a schedule, breaks your goals into steps, reads your files and answers questions about them, and keeps everything you have logged in one searchable place.', 'getting-started', 2, true, array['what is ionexa', 'what is this', 'what does it do', 'what can it do', 'how does it work']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('contact-support', 'en', 'How do I contact you?', 'Settings > Support, or reply to any email we have sent you. A person reads it — say what you were trying to do and what happened instead, and we will come back to you.', 'account', 4, true, array['contact', 'support', 'help', 'talk to a human', 'customer service', 'email you']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('credits-rollover', 'el', 'Μεταφέρονται τα credits στον επόμενο μήνα;', 'Τα μηνιαία credits του πλάνου ανανεώνονται κάθε μήνα και δεν συσσωρεύονται. Τα credits που αγοράζεις σε πακέτο είναι διαφορετικά: μένουν στον λογαριασμό σου μέχρι να τα χρησιμοποιήσεις.', 'credits', 3, true, array['μεταφερονται', 'rollover', 'χανονται τα credits', 'συσσωρευονται', 'expire credits']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('languages', 'el', 'Σε ποιες γλώσσες δουλεύει;', 'Το περιβάλλον υπάρχει σε δέκα γλώσσες και το αλλάζεις από τις Ρυθμίσεις. Το AI απαντά στη γλώσσα που του γράφεις, ανεξάρτητα από τη γλώσσα του περιβάλλοντος.', 'getting-started', 0, true, array['γλωσσα', 'γλωσσες', 'language', 'αγγλικα', 'ελληνικα', 'μεταφραση']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('password-reset', 'el', 'Ξέχασα τον κωδικό μου.', 'Στη σελίδα σύνδεσης πάτα «Ξέχασα τον κωδικό μου» και βάλε το email σου. Θα σου έρθει σύνδεσμος επαναφοράς. Αν δεν φτάνει, κοίτα και στα ανεπιθύμητα.', 'account', 1, true, array['ξεχασα τον κωδικο', 'password', 'reset password', 'επαναφορα κωδικου', 'δεν μπορω να μπω']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('contact-support', 'el', 'Πώς επικοινωνώ με άνθρωπο;', 'Ρυθμίσεις > Υποστήριξη, και γράψε μας. Αν το θέμα αφορά χρέωση, βάλε και τον αριθμό της χρέωσης για να το βρούμε γρήγορα.', 'account', 4, true, array['επικοινωνια', 'support', 'ανθρωπο', 'υποστηριξη', 'contact', 'να μιλησω με καποιον']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'es', '¿Qué es Ionexa?', 'Un espacio de trabajo donde la IA hace el trabajo en vez de solo aconsejarle: crea su web, ejecuta agentes con una periodicidad fija, divide sus objetivos en pasos, lee sus archivos y responde sobre ellos, y guarda todo lo que registra en un único sitio donde se puede buscar.', 'getting-started', 2, true, array['qué es ionexa', 'qué es esto', 'para qué sirve', 'qué hace', 'cómo funciona']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-agent', 'es', '¿Cómo creo un agente?', 'En el menú Agentes, describa en una frase qué quiere que se haga y cada cuánto: por ejemplo «cada mañana envíame un resumen de las noticias de mi sector». Ionexa construye el agente solo, lo ejecuta a su hora y le envía el resultado. Puede pausarlo o cambiarlo cuando quiera.', 'agents', 0, true, array['crear agente', 'hacer un agente', 'agentes', 'automatizar', 'automatización']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'es', '¿Cómo creo una misión?', 'En Mission Control escribe un objetivo como se lo diría a una persona: «quiero más clientes para la primavera». Ionexa lo divide en pasos concretos, y luego puede trabajarlos usted o encargar alguno a un agente.', 'missions', 0, true, array['crear misión', 'mission control', 'objetivo', 'objetivos', 'misiones']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'es', '¿Cómo conecto Gmail o Google Drive?', 'Ajustes > Conexiones. Elija el servicio y apruebe el acceso en la ventana de Google. Ve exactamente qué va a leer antes de aprobarlo, y puede desconectarlo cuando quiera: las claves de acceso se borran de inmediato.', 'integrations', 0, true, array['gmail', 'google drive', 'conectar', 'conexión', 'slack']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'es', '¿Cómo borro mi cuenta?', 'Ajustes > Cuenta > Eliminar cuenta. Se le pedirá confirmación y después se borra todo: conversaciones, archivos, webs, agentes, historial. No tiene vuelta atrás. Si quiere una copia antes, descargue sus datos desde la misma página.', 'privacy', 0, true, array['borrar cuenta', 'eliminar cuenta', 'cerrar cuenta', 'darme de baja del todo']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'fr', 'Comment changer de formule ?', 'Paramètres > Facturation. Choisissez la formule voulue et le changement prend effet immédiatement. En montée de gamme vous ne payez que la différence pour le reste de la période ; en descente vous gardez la formule actuelle jusqu''à la fin de la période déjà payée.', 'billing', 1, true, array['changer de formule', 'changer d''offre', 'passer au supérieur', 'rétrograder']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('upload-files', 'fr', 'Puis-je téléverser des fichiers ?', 'Oui : PDF, Word, Excel, CSV, texte et Markdown. Vous les déposez dans Fichiers, Ionexa lit leur contenu, puis vous pouvez poser des questions dessus. Vos fichiers sont privés : personne d''autre n''y a accès et chaque téléchargement passe par un lien temporaire.', 'files', 0, true, array['téléverser', 'importer un fichier', 'pdf', 'fichiers', 'documents']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'fr', 'Comment supprimer mon compte ?', 'Paramètres > Compte > Supprimer le compte. Une confirmation vous est demandée, puis tout est supprimé : conversations, fichiers, sites, agents, historique. C''est irréversible. Si vous voulez une copie avant, téléchargez vos données depuis la même page.', 'privacy', 0, true, array['supprimer mon compte', 'effacer mon compte', 'fermer mon compte', 'supprimer le compte']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-are-credits', 'de', 'Was sind Credits?', 'Credits sind die Währung, mit der du KI-Aktionen bezahlst: eine Chat-Nachricht, das Erzeugen einer Website, ein Agent-Lauf. Jeder Tarif enthält ein monatliches Kontingent, das sich erneuert, und du kannst Pakete nachkaufen, wenn es früher aufgebraucht ist. Wie viel jeder Tarif enthält, steht auf /pricing.', 'credits', 0, true, array['was sind credits', 'credits', 'wie funktionieren credits', 'einheiten']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('credits-ran-out', 'de', 'Meine Credits sind aufgebraucht — was jetzt?', 'Zwei Möglichkeiten: auf die monatliche Erneuerung warten oder ein Credit-Paket kaufen, das sofort gutgeschrieben wird und zum Monatsende nicht verfällt. Beides in Einstellungen > Abrechnung.', 'credits', 2, true, array['keine credits mehr', 'credits aufgebraucht', 'credits kaufen', 'mehr credits']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'de', 'Wie wechsle ich den Tarif?', 'Einstellungen > Abrechnung. Wähl den gewünschten Tarif, die Änderung gilt sofort. Beim Upgrade zahlst du nur die Differenz für den Rest der Periode; beim Downgrade behältst du den aktuellen Tarif bis zum Ende der bereits bezahlten Periode.', 'billing', 1, true, array['tarif wechseln', 'upgrade', 'downgrade', 'abo ändern']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('data-privacy', 'de', 'Was macht ihr mit meinen Daten?', 'Deine Daten gehören dir. Wir verkaufen sie nicht, wir trainieren keine Modelle damit, und jedes Konto sieht nur die eigenen — das wird in der Datenbank erzwungen, nicht bloß in der Anwendung. Du kannst sie jederzeit herunterladen oder löschen.', 'privacy', 2, true, array['meine daten', 'datenschutz', 'sicherheit', 'verkauft ihr meine daten', 'modelle trainieren']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'it', 'Che cos''è Ionexa?', 'Uno spazio di lavoro dove l''AI fa il lavoro invece di limitarsi a consigliare: crea il tuo sito, esegue agent secondo una pianificazione, divide i tuoi obiettivi in passi, legge i tuoi file e risponde su di essi, e tiene tutto ciò che registri in un unico posto ricercabile.', 'getting-started', 2, true, array['cos''è ionexa', 'che cos''è', 'a cosa serve', 'cosa fa', 'come funziona']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-website', 'it', 'Come faccio un sito?', 'Apri Website Builder dal menù laterale, scrivi a parole tue cosa vuoi — cosa fa la tua attività, a chi si rivolge, che stile ti piace — e avvia la creazione. Puoi anche caricare immagini di riferimento, un logo o foto di prodotto, così segue il tuo stile. Quando è pronto lo pubblichi a un indirizzo suo con un clic.', 'websites', 0, true, array['fare un sito', 'creare sito', 'sito web', 'voglio un sito', 'website builder']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'it', 'Come collego Gmail o Google Drive?', 'Impostazioni > Connessioni. Scegli il servizio e approva l''accesso nella finestra di Google. Vedi esattamente cosa verrà letto prima di approvare, e puoi scollegare quando vuoi — le chiavi di accesso vengono cancellate subito.', 'integrations', 0, true, array['gmail', 'google drive', 'collegare', 'connessione', 'slack']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'it', 'Come cancello il mio account?', 'Impostazioni > Account > Elimina account. Ti verrà chiesta conferma, poi viene cancellato tutto: conversazioni, file, siti, agent, cronologia. Non è reversibile. Se prima vuoi una copia, scarica i tuoi dati dalla stessa pagina.', 'privacy', 0, true, array['cancellare account', 'eliminare account', 'chiudere l''account', 'cancellare profilo']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'pt', 'O que é o Ionexa?', 'Um espaço de trabalho onde a IA faz o trabalho em vez de apenas aconselhar: cria o seu site, corre agentes segundo um horário, divide os seus objetivos em passos, lê os seus ficheiros e responde sobre eles, e guarda tudo o que regista num único sítio pesquisável.', 'getting-started', 2, true, array['o que é o ionexa', 'o que é isto', 'para que serve', 'o que faz', 'como funciona']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'pt', 'Como crio uma missão?', 'No Mission Control escreve um objetivo como o diria a uma pessoa: «quero mais clientes até à primavera». O Ionexa divide-o em passos concretos, e depois pode trabalhá-los você ou entregar alguns a um agente.', 'missions', 0, true, array['criar missão', 'mission control', 'objetivo', 'objetivos', 'missões']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'zh', 'Ionexa 是什么？', '一个由 AI 直接干活、而不只是给建议的工作空间：它替你做网站、按时运行 agent、把目标拆成步骤、读你的文件并回答相关问题，还把你记录过的一切放在一个可搜索的地方。', 'getting-started', 2, true, array['ionexa是什么', '这是什么', '有什么用', '能做什么', '怎么用']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('pricing-overview', 'zh', 'Ionexa 多少钱？', '有免费套餐可以先试，需求变大后再上付费套餐。当前价格和各套餐包含的内容始终在 /pricing 页面——这里不写具体数字，因为价格会变，写过时的数字比不写更糟。', 'billing', 0, true, array['多少钱', '价格', '收费', '套餐', '订阅']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-are-credits', 'zh', '什么是额度（credits）？', '额度是你为 AI 操作付费的方式：一条聊天消息、生成一个网站、一次 agent 运行。每个套餐都含每月自动续的额度，用得快也可以另外买加量包。各套餐含多少，见 /pricing。', 'credits', 0, true, array['什么是额度', 'credits', '额度怎么算', '点数']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('credits-ran-out', 'zh', '额度用完了怎么办？', '两个选择：等下个月自动续，或者买一个额度包——立刻到账，而且月底不会清零。两者都在「设置 > 账单」里。', 'credits', 2, true, array['额度用完', '没额度了', '买额度', '加额度']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'zh', '怎么换套餐？', '设置 > 账单。选你想要的套餐，立即生效。升级时只补本周期剩余部分的差价；降级时保留当前套餐直到已付费的周期结束。', 'billing', 1, true, array['换套餐', '升级', '降级', '改订阅']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('cancel', 'zh', '怎么取消订阅？', '设置 > 账单 > 取消订阅。不会立刻失去权限：套餐会用到已付费周期结束，之后账号转为免费套餐。你的数据都在——取消不会删除任何东西。', 'billing', 2, true, array['取消', '取消订阅', '退订', '停止订阅']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-website', 'zh', '怎么做一个网站？', '从侧边栏打开 Website Builder，用大白话写清你要什么——你的生意做什么、面向谁、喜欢什么风格——然后点生成。你也可以上传参考图，比如 logo 或产品照片，让它贴合你的视觉。做好之后，一键发布到它自己的地址。', 'websites', 0, true, array['做网站', '建网站', '网页', '我想要个网站', 'website builder']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-agent', 'zh', '怎么创建 agent？', '在 Agents 菜单里，用一句话说明要做什么、多久做一次——比如「每天早上给我发一份行业新闻摘要」。Ionexa 会自己搭好这个 agent，按时运行，并把结果发给你。你随时可以暂停或修改。', 'agents', 0, true, array['创建agent', '做一个agent', 'agents', '自动化', '定时任务']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'zh', '怎么创建 mission？', '在 Mission Control 里，像对人说话那样写下目标：「我想在春天前多拿些客户」。Ionexa 会把它拆成具体步骤，之后你可以自己做，也可以把其中几步交给 agent。', 'missions', 0, true, array['创建mission', 'mission control', '目标', '定目标', '任务']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('upload-files', 'zh', '可以上传文件吗？', '可以——PDF、Word、Excel、CSV、纯文本和 Markdown。在「文件」里上传，Ionexa 读取其中内容，之后你就能针对这些文件提问。你的文件是私有的：别人无法访问，每次下载都用临时链接。', 'files', 0, true, array['上传文件', '上传', 'pdf', '文件', '文档']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'zh', '怎么连接 Gmail 或 Google Drive？', '设置 > 连接。选择服务，在 Google 自己的窗口里授权。授权之前你能看清它将读取什么，而且随时可以断开——断开时访问密钥立刻删除。', 'integrations', 0, true, array['gmail', 'google drive', '连接', '集成', 'slack']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('data-privacy', 'zh', '你们拿我的数据做什么？', '你的数据是你的。我们不卖，也不拿来训练模型，而且每个账号只能看到自己的——这一点在数据库层面强制执行，不只是在应用里。你随时可以下载或删除。', 'privacy', 2, true, array['我的数据', '隐私', '安全', '会卖我的数据吗', '训练模型']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'zh', '怎么注销账号？', '设置 > 账号 > 删除账号。会要求你确认，之后全部删除：对话、文件、网站、agent、历史记录。不可恢复。如果想先留一份备份，在同一页面下载你的数据。', 'privacy', 0, true, array['删除账号', '注销账号', '关闭账号', '销号']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'ja', 'Ionexaとは？', 'AIが助言するだけでなく実際に作業するワークスペースです。サイトを作り、エージェントを決めた頻度で走らせ、目標を手順に分け、ファイルを読んでそれについて答え、記録したすべてを検索できる一か所にまとめます。', 'getting-started', 2, true, array['ionexaとは', 'これは何', '何ができる', '何をするもの', '使い方']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('pricing-overview', 'ja', 'Ionexaの料金は？', 'まず試せる無料プランがあり、必要に応じて有料プランに移れます。現在の価格と各プランの内容は常に /pricing にあります。ここに数字を書かないのは、変わるからです。古い数字は書かないより悪いものです。', 'billing', 0, true, array['いくら', '料金', '価格', 'プラン', 'サブスク']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-are-credits', 'ja', 'クレジットとは？', 'クレジットはAIの処理に対して支払う通貨です。チャットのメッセージ、サイトの生成、エージェントの実行などに使います。各プランには毎月更新される割り当てがあり、早く使い切った場合は追加パックを購入できます。プランごとの量は /pricing にあります。', 'credits', 0, true, array['クレジットとは', 'クレジット', 'クレジットの仕組み', 'ポイント']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('credits-ran-out', 'ja', 'クレジットがなくなりました', '方法はふたつです。翌月の更新を待つか、クレジットパックを購入するか。パックはすぐ反映され、月末で失効しません。どちらも「設定 > 請求」から行えます。', 'credits', 2, true, array['クレジットがない', 'クレジット切れ', 'クレジットを買う', '追加したい']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'ja', 'プランはどう変更しますか？', '設定 > 請求。希望のプランを選ぶと、変更はすぐ反映されます。上位に変える場合は残り期間の差額のみ、下位に変える場合は支払い済み期間の終わりまで現在のプランのままです。', 'billing', 1, true, array['プラン変更', 'アップグレード', 'ダウングレード', '契約を変える']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('cancel', 'ja', '解約はどうしますか？', '設定 > 請求 > 解約。すぐに使えなくなるわけではありません。支払い済み期間の終わりまでは現在のプランが続き、その後は無料プランに切り替わります。データはそのまま残り、解約で削除されるものはありません。', 'billing', 2, true, array['解約', 'キャンセル', '退会したい', '契約をやめる']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-website', 'ja', 'サイトはどう作りますか？', 'サイドバーからWebsite Builderを開き、ふつうの言葉で書いてください。事業の内容、対象、好みの雰囲気などです。参考画像（ロゴや商品写真）も追加でき、見た目を合わせられます。完成したら、ワンクリックで専用のアドレスに公開できます。', 'websites', 0, true, array['サイトを作る', 'ホームページ', 'ウェブサイト作成', 'サイトが欲しい', 'website builder']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-agent', 'ja', 'エージェントはどう作りますか？', 'Agentsメニューで、やってほしいことと頻度を一文で書きます。たとえば「毎朝、業界ニュースの要約を送って」。Ionexaがエージェントを組み立て、時刻どおりに実行し、結果を届けます。いつでも停止・変更できます。', 'agents', 0, true, array['エージェントを作る', 'agent作成', 'エージェント', '自動化', '定期実行']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'ja', 'ミッションはどう作りますか？', 'Mission Controlで、人に話すように目標を書きます。「春までに顧客を増やしたい」のように。Ionexaがそれを具体的な手順に分解し、そのあとは自分で進めても、いくつかをエージェントに任せてもかまいません。', 'missions', 0, true, array['ミッションを作る', 'mission control', '目標', 'ゴール', 'ミッション']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('upload-files', 'ja', 'ファイルはアップロードできますか？', 'はい。PDF、Word、Excel、CSV、テキスト、Markdownに対応しています。Filesにアップロードすると内容が読み取られ、そのファイルについて質問できます。ファイルは非公開で、他の人はアクセスできず、ダウンロードは毎回一時リンク経由です。', 'files', 0, true, array['ファイルをアップロード', 'アップロード', 'pdf', 'ファイル', '資料']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'ja', 'GmailやGoogle Driveはどう接続しますか？', '設定 > 連携。サービスを選び、Google自身の画面で許可します。許可の前に何を読むのかが正確に表示され、いつでも接続を解除できます。解除すると、アクセスキーはただちに削除されます。', 'integrations', 0, true, array['gmail', 'google drive', '接続', '連携', 'slack']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('data-privacy', 'ja', 'データはどう扱われますか？', 'あなたのデータはあなたのものです。販売しません。モデルの学習にも使いません。各アカウントは自分のデータしか見られず、それはアプリだけでなくデータベース側で強制されています。いつでもダウンロードも削除もできます。', 'privacy', 2, true, array['私のデータ', 'プライバシー', 'セキュリティ', 'データを売る', '学習に使う']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'ja', 'アカウントはどう削除しますか？', '設定 > アカウント > アカウント削除。確認を求められ、そのあとすべて削除されます。会話、ファイル、サイト、エージェント、履歴。取り消せません。先に控えが必要なら、同じページからデータを書き出してください。', 'privacy', 0, true, array['アカウント削除', '退会', 'アカウントを消す', '解約して消したい']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('what-is-ionexa', 'ar', 'ما هو Ionexa؟', 'مساحة عمل ينفّذ فيها الذكاء الاصطناعي العمل بدل أن يكتفي بالنصيحة: يبني موقعك، ويشغّل الوكلاء وفق جدول زمني، ويقسّم أهدافك إلى خطوات، ويقرأ ملفاتك ويجيب عنها، ويحفظ كل ما سجّلته في مكان واحد قابل للبحث.', 'getting-started', 2, true, array['ما هو ionexa', 'ما هذا', 'ما فائدته', 'ماذا يفعل', 'كيف يعمل']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('pricing-overview', 'ar', 'كم يكلّف Ionexa؟', 'هناك خطة مجانية للتجربة، وخطط مدفوعة كلما كبرت احتياجاتك. الأسعار الحالية وما تتضمّنه كل خطة موجودة دائمًا في صفحة ‎/pricing‎ — ولا نكتبها هنا لأنها تتغيّر، ورقم قديم أسوأ من لا رقم.', 'billing', 0, true, array['كم التكلفة', 'السعر', 'الأسعار', 'الخطط', 'الاشتراك']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('credits-ran-out', 'ar', 'نفدت الـcredits، ماذا أفعل؟', 'خياران: انتظار التجديد الشهري، أو شراء باقة credits تُضاف فورًا ولا تنتهي في آخر الشهر. كلاهما من الإعدادات > الفوترة.', 'credits', 2, true, array['نفدت الcredits', 'لا يوجد رصيد', 'شراء credits', 'أريد المزيد']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('change-plan', 'ar', 'كيف أغيّر الخطة؟', 'الإعدادات > الفوترة. اختر الخطة التي تريدها ويسري التغيير فورًا. عند الترقية تدفع الفارق فقط لبقية الفترة؛ وعند النزول تحتفظ بخطتك الحالية حتى تنتهي الفترة المدفوعة.', 'billing', 1, true, array['تغيير الخطة', 'ترقية', 'تخفيض الخطة', 'تغيير الاشتراك']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('cancel', 'ar', 'كيف ألغي اشتراكي؟', 'الإعدادات > الفوترة > إلغاء الاشتراك. لا تفقد الوصول فورًا: تستمر خطتك حتى نهاية الفترة المدفوعة، ثم يتحوّل الحساب إلى الخطة المجانية. بياناتك تبقى — الإلغاء لا يحذف شيئًا.', 'billing', 2, true, array['إلغاء', 'إلغاء الاشتراك', 'إيقاف الاشتراك', 'أريد الإلغاء']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-website', 'ar', 'كيف أنشئ موقعًا؟', 'افتح Website Builder من القائمة الجانبية، واكتب بكلماتك ما تريد — ماذا يعمل نشاطك، ولمن، وأي أسلوب تفضّل — ثم اضغط إنشاء. يمكنك أيضًا رفع صور مرجعية، شعارًا أو صور منتجات، ليتبع هويتك. وعندما يجهز تنشره على عنوان خاص به بنقرة واحدة.', 'websites', 0, true, array['إنشاء موقع', 'عمل موقع', 'صفحة ويب', 'أريد موقعًا', 'website builder']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-agent', 'ar', 'كيف أنشئ وكيلًا؟', 'في قائمة الوكلاء، صِف في جملة واحدة ما تريد إنجازه وكل كم مرة — مثلًا «كل صباح أرسل لي ملخّصًا لأخبار قطاعي». يبني Ionexa الوكيل بنفسه، ويشغّله في موعده، ويرسل لك النتيجة. ويمكنك إيقافه أو تعديله متى شئت.', 'agents', 0, true, array['إنشاء وكيل', 'عمل agent', 'الوكلاء', 'أتمتة', 'مهمة متكررة']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('create-mission', 'ar', 'كيف أنشئ مهمة؟', 'في Mission Control تكتب هدفًا كما تقوله لإنسان: «أريد عملاء أكثر قبل الربيع». يقسّمه Ionexa إلى خطوات محدّدة، ثم تنفّذها بنفسك أو تُسنِد بعضها إلى وكيل.', 'missions', 0, true, array['إنشاء مهمة', 'mission control', 'هدف', 'أهداف', 'المهام']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('upload-files', 'ar', 'هل يمكنني رفع ملفات؟', 'نعم — PDF وWord وExcel وCSV والنصوص وMarkdown. ترفعها في Files، فيقرأ Ionexa محتواها، ثم تستطيع طرح أسئلة عليها. ملفاتك خاصة: لا يصل إليها أحد غيرك، وكل تنزيل يتم عبر رابط مؤقّت.', 'files', 0, true, array['رفع ملف', 'تحميل ملف', 'pdf', 'الملفات', 'مستندات']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('connect-gmail', 'ar', 'كيف أربط Gmail أو Google Drive؟', 'الإعدادات > الاتصالات. اختر الخدمة ووافق على الوصول في نافذة Google نفسها. ترى بالضبط ما الذي سيُقرأ قبل الموافقة، ويمكنك فصل الاتصال متى شئت — وعندها تُحذف مفاتيح الوصول فورًا.', 'integrations', 0, true, array['gmail', 'google drive', 'ربط', 'اتصال', 'slack']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('data-privacy', 'ar', 'ماذا تفعلون ببياناتي؟', 'بياناتك ملكك. لا نبيعها، ولا نستخدمها لتدريب النماذج، وكل حساب لا يرى إلا بياناته — وهذا مفروض في قاعدة البيانات لا في التطبيق وحده. ويمكنك تنزيلها أو حذفها متى شئت.', 'privacy', 2, true, array['بياناتي', 'الخصوصية', 'الأمان', 'هل تبيعون بياناتي', 'تدريب النماذج']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
...
insert into public.help_articles (slug, locale, title, body, category, "order", published, triggers)
values ('delete-account', 'ar', 'كيف أحذف حسابي؟', 'الإعدادات > الحساب > حذف الحساب. سيُطلب منك التأكيد، ثم يُحذف كل شيء: المحادثات والملفات والمواقع والوكلاء والسجل. ولا يمكن التراجع. وإن أردت نسخة أولًا، نزّل بياناتك من الصفحة نفسها.', 'privacy', 0, true, array['حذف الحساب', 'إلغاء الحساب', 'إغلاق الحساب', 'أريد حذف حسابي']::text[])
on conflict (slug, locale) do update set
  title = excluded.title,
  body = excluded.body,
  category = excluded.category,
