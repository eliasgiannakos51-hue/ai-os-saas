-- ===========================================================================
-- GRANDFATHERING for the combined-ceiling change.
--
...
--
-- IT IS A DELIBERATE, BOUNDED BREACH OF THE CEILING, and it is worth being
-- explicit about the size: a grandfathered Professional account costs
-- EUR 18.82/month in worst-case free chat against a EUR 20.00 combined
-- budget, i.e. 38.8% of its revenue and a combined 2.58x rather than 4x.
-- That is the price of not taking something back. What is not acceptable
-- is paying it invisibly, so the cohort is countable — see the query at
-- the bottom of this file.
--
-- FOUR GUARDS, so the exception cannot outlive its reason:
--   1. legacy_plan_tier — the entitlement applies ONLY while the account is
--      still on the plan it was granted for. This is what stops a beta
--      account (plan_tier 'ultimate', silently collapsing to Free when
