...
--
-- SAFE TO RE-RUN. Every table is `create table if not exists`, so running
-- this file against a database that already holds data changes nothing
-- about that data.
--
-- IT WAS NOT ALWAYS. Until this change the file opened with 39
-- `drop table if exists ... cascade` statements covering every module
-- table plus chat_conversations, chat_messages, chat_memory, ai_missions,
-- user_websites, website_versions, ai_documents, ai_presentations and
-- more. A comment right here said DO NOT RE-RUN — which is not a
-- safeguard, it is a hope. Measured against PostgreSQL 16: 47 rows in
-- chat_memory, re-run this file's chat_memory block, 0 rows. That is
-- precisely the reported symptom "Settings says 0 things remembered
-- despite dozens of conversations", and it would have taken the
-- conversations themselves, the websites and the documents with it.
--
-- A file that is destructive when run twice, in a project whose whole
-- workflow is "paste the SQL into the Supabase editor", will eventually be
-- run twice. So it is no longer destructive.
--
-- THE ONE TRADE-OFF, stated plainly: because the CREATEs are now
-- `if not exists`, re-running this file will NOT restructure a table that
-- already exists with different columns. That is what the additive
-- `alter table ... add column if not exists` blocks throughout this file
-- are for, and it is the pattern every future change must follow. Losing a
-- restructure you can redo by hand beats losing data you cannot.
-- ============================================================================
